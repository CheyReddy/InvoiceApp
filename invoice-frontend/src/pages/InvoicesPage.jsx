...
import { getInvoices, deleteInvoice, sendInvoice, downloadInvoicePdf } from '../api/invoiceApi.js';
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white mt-2">Invoices</h1>
          </div>
              ...
              <tbody>
                {invoices.map((inv) => (
                  <tr key={inv.id} className="border-t border-gray-100 dark:border-gray-700">
                    ...
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{inv.dueDate}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[inv.status]}`}>
                        {inv.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right text-gray-900 dark:text-white">${Number(inv.total).toFixed(2)}</td>
